        ...
        private static void SeedQuests()
        {
            var quests = new List<QuestModel>
            {
                ...
                new QuestModel
                {
                    ...
                    SolutionCode = "int Max(int[] arr) { int m=arr[0]; for(int i=1;i<arr.Length;i++) if(arr[i]>m) m=arr[i]; return m; }",
                    Points = 120
                },
                new QuestModel
                {
                    Id = 5,
                    Title = "Взлом: SQL инъекция",
                    Type = "Hack",
                ...
                {
                    Id = 6,
                    Title = "Легаси: Пузырьковая сортировка",
                    Type = "LegacyFix",
                    ...
                    SolutionCode = "void Sort(int[] arr) { for(int i=0;i<arr.Length-1;i++) for(int j=0;j<arr.Length-i-1;j++) if(arr[j]>arr[j+1]) { int t=arr[j]; arr[j]=arr[j+1]; arr[j+1]=t; } }",
                    Points = 250
                },
                new QuestModel
                {
                    Id = 7,
                    Title = "Легаси: Палиндром",
                    Type = "LegacyFix",
                    ...
                    SolutionCode = "bool IsPal(string s) { for(int i=0;i<s.Length/2;i++) if(s[i]!=s[s.Length-1-i]) return false; return true; }",
                    Points = 130
                },
                new QuestModel
                {
                    Id = 8,
                    Title = "Взлом: JWT токен",
                    Type = "Hack",
